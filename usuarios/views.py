...
def fnct_enviar_email_restar_senha(request):
    if request.method == "GET":
        ...
        if user.exists():
            ...
            token = uuid.uuid4()
            Perfil_User.token = token  # Supondo que você tenha um campo 'token' no perfil do usuário
            Perfil_User.save()
           ...
           # return render(request, 'clientes_lista.html', {'RlcaoDClntes': slct_Clnts_Lsta})
            ...
            messages.add_message(request, constants.INFO, 'E-mail de redefinição de senha enviado com sucesso. Verifica seu eMail, para resetar sua senha...')
            return redirect('/usuarios/login')

        else:
            print("O email informado não existe!!!")
            messages.add_message(request, constants.ERROR, 'O eMail informado não se encontra cadastrado...')
    ...
    if request.method == "GET":
        ...
        try:
            if get_img_foto != 'None':
                ...
                path =  os.path.join(settings.BASE_DIR, f'media/user_photos/{get_img_foto.name}')
        ...
        if get_snha != get_cnfrmr_snha:
            print('Erro ')
            messages.add_message(request, constants.ERROR, 'Confirmação de senha não confere!')
            return redirect('/usuarios/users_profile')

        if len(get_firstname) < 6:
            print('Erro 3')
            messages.add_message(request, constants.ERROR, 'Digite um primeiro nome com no mínimo 7 caracters.')
            return redirect('/usuarios/users_profile')
        ...
        try:
            ...
            messages.add_message(request, constants.SUCCESS, 'Os Dados Salvo com sucesso...', 'bi bi-check-circle me-1')
            return redirect('/usuarios/users_profile')
            
        except:
            print('Erro 4')
            messages.add_message(request, constants.ERROR, 'Os dados não foram salvo com sucesso...', 'bi bi-exclamation-octagon me-1')
            return redirect('/usuarios/users_profile')
